...
# Read raw dataset
###################
raw_df = pd.read_csv("dataset.csv") # Load Dataset
param_df = pd.read_csv("parameters.csv", index_col="风机编号").T # Load turbine parameters
raw_df["label"] = 0
...
# 对每个风机设置不同的参数
recur_param_df = pd.read_csv("recur_param.csv", index_col="风机编号")
...
df = raw_df[raw_df["label"]==0]     #把目前正常的挑选出来
for wind_number, sub_df in df.groupby("WindNumber"):
...
df = raw_df[raw_df["label"]==0]
for wind_number, sub_df in df.groupby("WindNumber"):
    if ~(wind_number == 3 or wind_number == 6):
        continue
    ...
    idx = (sub_df["diff"] < 0) & (sub_df["WindSpeed"] > 0) & (sub_df["WindSpeed"] <= 10)
    index = sub_df[idx].index
    raw_df.loc[index, "label"] = 1
...
for wind_number, sub_df in raw_df.groupby("WindNumber"):
    ...
    ax.set_zlabel("Power")
    fig.colorbar(ax0)
    plt.savefig("./recur/" + str(wind_number) + "_results_scatter.jpg")
    plt.close()
...
for wind_number, sub_df in df.groupby("WindNumber"):
    ...
    if wind_number == 3:
        ...
        y = 8.968 * ((x - 0.5) ** 2) + 32.3 * (x - 0.5) - 259.1  # 函数表达式
        axs[0].plot(x, y, 'r', label='fitting')

    if wind_number == 6:
        x = np.linspace(0, 10, 500)
        y = -1.86 * ((x - 0.5) ** 2) + 160.1 * (x - 0.5) - 639.8  # 函数表达式
        axs[0].plot(x, y, 'r', label='fitting')
    ##
    ...
    plt.savefig("./recur/" + str(wind_number) + "_dim_relation.jpg")
    plt.close()
